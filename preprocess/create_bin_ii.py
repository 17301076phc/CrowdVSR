...
if __name__ == '__main__':
    ...
    # hr_patch_size = 256
    # scale = 2
    # hr_patch_size = 48*2#128 # the size is for hr patch
    scale = 4
    hr_patch_size = 48*4

    # lr_dir = os.path.join(args.dir_data, 'DIV2K/bin/DIV2K_train_LR_bicubic/X{}/'.format(scale))
    # hr_dir = os.path.join("candidates_bin", 'DIV2K/bin/DIV2K_train_HR/')
    # lr_dir = "candidates_lr/" + argpath
    # lr_dir = "candidates_lr_x2/" + argpath
    # lr_dir = "candidates_lr_x3/" + argpath
    #
    # hr_dir = "candidates/" + argpath
    lr_dir = "online_data_lr_x4"
    hr_dir = "online_data"
    ...
    for i, file in enumerate(files):
        ...
        # imageio.imsave("preprocess/tmp.png",psnr_map.astype(np.uint8))
        ...
        # print(index_psnr)
        nums_patches = int(index_psnr.shape[0]*0.1)
        # print(nums_patches)
        result = []
        result_file = Path(f'online_data_patch_x4'+f'/{str(i).zfill(4)}.txt').open('w', encoding='utf8')
        # result_file = Path(f'patch_hash_x3/'+argpath+f'/{str(i).zfill(4)}.txt').open('w', encoding='utf8')
